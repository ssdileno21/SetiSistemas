unit uUsuarioGruposControl;

interface

uses uUsuarioGruposDao, uUsuarioGruposModel, uMess, SysUtils, Forms, Buttons, StdCtrls,
Vcl.Graphics,Windows,Messages,Variants,Classes,Controls,Dialogs, DBGrids;

    type TUsuarioGruposControl = class
      private
          FcModel : TUsuarioGruposModel;
          FcDao : TUsuarioGruposDao;
      ...
      public
        {  constructor Create(Form:TObject);
          ...
          property cModel : TUsuarioGruposModel read FcModel write FcModel;
          property cDao : TUsuarioGruposDao read FcDao write FcDao;

    end;

implementation

{ TUsuarioGruposControl }

{constructor TUsuarioGruposControl.Create(Form: TObject);
begin
    try
...
procedure TUsuarioGruposControl.DbGridSetValue(Form: TObject);
begin
    try
       with (Form as TfrmForms) do
        begin
...
procedure TUsuarioGruposControl.ParamSet(Form: TObject;State:Integer);
var
    pStatus : string;
begin
    try
...
procedure TUsuarioGruposControl.Search(Form: TObject);
begin
  try
     with (Form as TfrmForms) do
      begin
...
procedure TUsuarioGruposControl.BackRecords(Form:TObject);
begin
    try
      with (Form as TfrmForms) do
      begin
...
procedure TUsuarioGruposControl.Cancel(Form: TObject);
begin
    try
...
procedure TUsuarioGruposControl.Save(Form: TObject);
begin
    try
      ParamSet(Form,6);
      Control(1,Form);
...
procedure TUsuarioGruposControl.Delete(Form: TObject);
begin
    try
 //     Control(2,Form);
    except on e : exception do
...
procedure TUsuarioGruposControl.New(Form: TObject);
begin
    try
      ParamSet(Form,3);
      Control(3,Form);
...
procedure TUsuarioGruposControl.Edit(Form: TObject);
begin
    try
      ParamSet(Form,4);
      Control(4,Form);
...
procedure TUsuarioGruposControl.Clear(Form: TObject);
begin
    try
      Control(5,Form);
    except on e : exception do
...
procedure TUsuarioGruposControl.Control(State: Integer; Form: TObject);
var
    i : Integer;
begin
  try
