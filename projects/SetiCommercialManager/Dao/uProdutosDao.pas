...
uses uProdutosModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TProdutosDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TProdutosModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TProdutosModel);
          procedure Edit(Model:TProdutosModel);
          procedure Delete(Model:TProdutosModel);
          procedure Save(Model:TProdutosModel);
...
procedure TProdutosDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TProdutosDao.Delete(Model: TProdutosModel);
begin
...
procedure TProdutosDao.Edit(Model: TProdutosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TProdutosDao.FormList(Model: TProdutosModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TProdutosDao.Insert(Model: TProdutosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TProdutosDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TProdutosDao.Save(Model: TProdutosModel);
begin
    try
