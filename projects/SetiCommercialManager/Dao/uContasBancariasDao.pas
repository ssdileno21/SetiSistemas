unit uContasBancariasDao;

interface

uses uContasBancariasModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TContasBancariasDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TContasBancariasModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TContasBancariasModel);
          procedure Edit(Model:TContasBancariasModel);
          procedure Delete(Model:TContasBancariasModel);
          procedure Save(Model:TContasBancariasModel);
...
procedure TContasBancariasDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TContasBancariasDao.Create;
begin
    try
...
procedure TContasBancariasDao.Delete(Model: TContasBancariasModel);
begin
...
destructor TContasBancariasDao.Destroy;
begin
    try
...
procedure TContasBancariasDao.Edit(Model: TContasBancariasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TContasBancariasDao.FormList(Model: TContasBancariasModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TContasBancariasDao.Insert(Model: TContasBancariasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TContasBancariasDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TContasBancariasDao.Save(Model: TContasBancariasModel);
begin
    try
