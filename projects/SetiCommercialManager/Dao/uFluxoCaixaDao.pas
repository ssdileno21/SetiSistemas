...
uses uFluxoCaixaModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TFluxoCaixaDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TFluxoCaixaModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TFluxoCaixaModel);
          procedure Edit(Model:TFluxoCaixaModel);
          procedure Delete(Model:TFluxoCaixaModel);
          procedure Save(Model:TFluxoCaixaModel);
...
procedure TFluxoCaixaDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TFluxoCaixaDao.Create;
begin
    try
...
procedure TFluxoCaixaDao.Delete(Model: TFluxoCaixaModel);
begin
...
procedure TFluxoCaixaDao.Edit(Model: TFluxoCaixaModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TFluxoCaixaDao.FormList(Model: TFluxoCaixaModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TFluxoCaixaDao.Insert(Model: TFluxoCaixaModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TFluxoCaixaDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TFluxoCaixaDao.Save(Model: TFluxoCaixaModel);
begin
    try
