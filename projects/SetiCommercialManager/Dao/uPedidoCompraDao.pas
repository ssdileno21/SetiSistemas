unit uPedidoCompraDao;

interface

uses uPedidoCompraModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TPedidoCompraDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TPedidoCompraModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TPedidoCompraModel);
          procedure Edit(Model:TPedidoCompraModel);
          procedure Delete(Model:TPedidoCompraModel);
          procedure Save(Model:TPedidoCompraModel);
...
procedure TPedidoCompraDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TPedidoCompraDao.Create;
begin
    try
...
procedure TPedidoCompraDao.Delete(Model: TPedidoCompraModel);
begin
...
procedure TPedidoCompraDao.Edit(Model: TPedidoCompraModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TPedidoCompraDao.FormList(Model: TPedidoCompraModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TPedidoCompraDao.Insert(Model: TPedidoCompraModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TPedidoCompraDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TPedidoCompraDao.Save(Model: TPedidoCompraModel);
begin
    try
