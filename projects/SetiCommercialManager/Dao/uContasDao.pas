...
uses uContasModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TContasDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TContasModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TContasModel);
          procedure Edit(Model:TContasModel);
          procedure Delete(Model:TContasModel);
          procedure Save(Model:TContasModel);
...
procedure TContasDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TContasDao.Delete(Model: TContasModel);
begin
...
procedure TContasDao.Edit(Model: TContasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TContasDao.FormList(Model: TContasModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TContasDao.Insert(Model: TContasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TContasDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TContasDao.Save(Model: TContasModel);
begin
    try
