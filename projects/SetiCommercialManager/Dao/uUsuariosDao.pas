...
uses uUsuariosModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TUsuariosDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TUsuariosModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TUsuariosModel);
          procedure Edit(Model:TUsuariosModel);
          procedure Delete(Model:TUsuariosModel);
          procedure Save(Model:TUsuariosModel);
...
procedure TUsuariosDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TUsuariosDao.Delete(Model: TUsuariosModel);
begin
...
procedure TUsuariosDao.Edit(Model: TUsuariosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TUsuariosDao.FormList(Model: TUsuariosModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TUsuariosDao.Insert(Model: TUsuariosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TUsuariosDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TUsuariosDao.Save(Model: TUsuariosModel);
begin
    try
