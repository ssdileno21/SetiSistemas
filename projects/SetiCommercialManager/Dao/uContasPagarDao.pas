...
uses uContasPagarModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TContasPagarDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TContasPagarModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TContasPagarModel);
          procedure Edit(Model:TContasPagarModel);
          procedure Delete(Model:TContasPagarModel);
          procedure Save(Model:TContasPagarModel);
...
procedure TContasPagarDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TContasPagarDao.Delete(Model: TContasPagarModel);
begin
...
procedure TContasPagarDao.Edit(Model: TContasPagarModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TContasPagarDao.FormList(Model: TContasPagarModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TContasPagarDao.Insert(Model: TContasPagarModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TContasPagarDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TContasPagarDao.Save(Model: TContasPagarModel);
begin
    try
