...
uses uOrcamentosModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TOrcamentosDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TOrcamentosModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TOrcamentosModel);
          procedure Edit(Model:TOrcamentosModel);
          procedure Delete(Model:TOrcamentosModel);
          procedure Save(Model:TOrcamentosModel);
...
procedure TOrcamentosDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TOrcamentosDao.Create;
begin
    try
...
procedure TOrcamentosDao.Delete(Model: TOrcamentosModel);
begin
...
destructor TOrcamentosDao.Destroy;
begin
    try
...
procedure TOrcamentosDao.Edit(Model: TOrcamentosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TOrcamentosDao.FormList(Model: TOrcamentosModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TOrcamentosDao.Insert(Model: TOrcamentosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TOrcamentosDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TOrcamentosDao.Save(Model: TOrcamentosModel);
begin
    try
