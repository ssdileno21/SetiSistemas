...
uses uPedidoVendaModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TPedidoVendaDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TPedidoVendaModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TPedidoVendaModel);
          procedure Edit(Model:TPedidoVendaModel);
          procedure Delete(Model:TPedidoVendaModel);
          procedure Save(Model:TPedidoVendaModel);
...
procedure TPedidoVendaDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TPedidoVendaDao.Delete(Model: TPedidoVendaModel);
begin
...
procedure TPedidoVendaDao.Edit(Model: TPedidoVendaModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TPedidoVendaDao.FormList(Model: TPedidoVendaModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TPedidoVendaDao.Insert(Model: TPedidoVendaModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TPedidoVendaDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TPedidoVendaDao.Save(Model: TPedidoVendaModel);
begin
    try
