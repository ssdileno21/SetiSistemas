...
uses uClientesModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TClientesDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TClientesModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TClientesModel);
          procedure Edit(Model:TClientesModel);
          procedure Delete(Model:TClientesModel);
          procedure Save(Model:TClientesModel);
...
procedure TClientesDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TClientesDao.Delete(Model: TClientesModel);
begin
...
procedure TClientesDao.Edit(Model: TClientesModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TClientesDao.FormList(Model: TClientesModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TClientesDao.Insert(Model: TClientesModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TClientesDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TClientesDao.Save(Model: TClientesModel);
begin
    try
