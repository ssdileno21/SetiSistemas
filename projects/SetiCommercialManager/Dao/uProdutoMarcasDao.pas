unit uProdutoMarcasDao;

interface

uses uProdutoMarcasModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TProdutoMarcasDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TProdutoMarcasModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TProdutoMarcasModel);
          procedure Edit(Model:TProdutoMarcasModel);
          procedure Delete(Model:TProdutoMarcasModel);
          procedure Save(Model:TProdutoMarcasModel);
...
procedure TProdutoMarcasDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TProdutoMarcasDao.Create;
begin
    try
...
procedure TProdutoMarcasDao.Delete(Model: TProdutoMarcasModel);
begin
...
destructor TProdutoMarcasDao.Destroy;
begin
    try
...
procedure TProdutoMarcasDao.Edit(Model: TProdutoMarcasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TProdutoMarcasDao.FormList(Model: TProdutoMarcasModel; Grid: TDBGrid)
: Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TProdutoMarcasDao.Insert(Model: TProdutoMarcasModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TProdutoMarcasDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TProdutoMarcasDao.Save(Model: TProdutoMarcasModel);
begin
    try
