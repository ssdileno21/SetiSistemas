...
uses uFornecedoresModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TFornecedoresDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TFornecedoresModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TFornecedoresModel);
          procedure Edit(Model:TFornecedoresModel);
          procedure Delete(Model:TFornecedoresModel);
          procedure Save(Model:TFornecedoresModel);
...
procedure TFornecedoresDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
procedure TFornecedoresDao.Delete(Model: TFornecedoresModel);
begin
...
procedure TFornecedoresDao.Edit(Model: TFornecedoresModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TFornecedoresDao.FormList(Model: TFornecedoresModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TFornecedoresDao.Insert(Model: TFornecedoresModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
      //
...
procedure TFornecedoresDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TFornecedoresDao.Save(Model: TFornecedoresModel);
begin
    try
