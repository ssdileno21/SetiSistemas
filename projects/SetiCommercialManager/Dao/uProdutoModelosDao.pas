unit uProdutoModelosDao;

interface

uses uProdutoModelosModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TProdutoModelosDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TProdutoModelosModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TProdutoModelosModel);
          procedure Edit(Model:TProdutoModelosModel);
          procedure Delete(Model:TProdutoModelosModel);
          procedure Save(Model:TProdutoModelosModel);
...
procedure TProdutoModelosDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TProdutoModelosDao.Create;
begin
    try
...
procedure TProdutoModelosDao.Delete(Model: TProdutoModelosModel);
begin
...
destructor TProdutoModelosDao.Destroy;
begin
    try
...
procedure TProdutoModelosDao.Edit(Model: TProdutoModelosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TProdutoModelosDao.FormList(Model: TProdutoModelosModel; Grid: TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
begin
...
procedure TProdutoModelosDao.Insert(Model: TProdutoModelosModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TProdutoModelosDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TProdutoModelosDao.Save(Model: TProdutoModelosModel);
begin
    try
