unit uUsuarioGruposDao;

interface

uses uUsuarioGruposModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TUsuarioGruposDao = class(TProvider)
      private
...
//          procedure ProviderFlags(DataSet:TDataSet);
      ...
      public
          ...
          //function FormList(Model:TUsuarioGruposModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TUsuarioGruposModel);
          procedure Edit(Model:TUsuarioGruposModel);
          procedure Delete(Model:TUsuarioGruposModel);
          procedure Save(Model:TUsuarioGruposModel);
...
{procedure TUsuarioGruposDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TUsuarioGruposDao.Create;
begin
    try
...
procedure TUsuarioGruposDao.Delete(Model: TUsuarioGruposModel);
begin
...
destructor TUsuarioGruposDao.Destroy;
begin
    try
...
procedure TUsuarioGruposDao.Edit(Model: TUsuarioGruposModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
{function TUsuarioGruposDao.FormList(Model: TUsuarioGruposModel; Grid: TDBGrid) :
Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TUsuarioGruposDao.Insert(Model: TUsuarioGruposModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
{procedure TUsuarioGruposDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      with (DataSet) do
      begin
...
procedure TUsuarioGruposDao.Save(Model: TUsuarioGruposModel);
begin
    try
