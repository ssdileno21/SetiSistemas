unit uTransportadoresDao;

interface

uses uTransportadoresModel, uMess, SysUtils, uSetiConfig, uSetiConnects, SqlExpr,
ADODB, DB, Provider, DBClient, DBGrids, System.Variants, Dialogs;

    type TTransportadoresDao = class(TProvider)
      private
      ...
      public
          ...
          function FormList(Model:TTransportadoresModel;Grid:TDBGrid) : Boolean;
          procedure Insert(Model:TTransportadoresModel);
          procedure Edit(Model:TTransportadoresModel);
          procedure Delete(Model:TTransportadoresModel);
          procedure Save(Model:TTransportadoresModel);
...
procedure TTransportadoresDao.CompCreate(Create:Boolean);
var
    pDb : string;
begin
    try
...
constructor TTransportadoresDao.Create;
begin
    try
...
procedure TTransportadoresDao.Delete(Model: TTransportadoresModel);
begin
...
destructor TTransportadoresDao.Destroy;
begin
    try
...
procedure TTransportadoresDao.Edit(Model: TTransportadoresModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
function TTransportadoresDao.FormList(Model: TTransportadoresModel; Grid:
TDBGrid) : Boolean;
var
    pVal : Boolean;
    pDb : string;
...
procedure TTransportadoresDao.Insert(Model: TTransportadoresModel);
begin
    //STATE: 1: INICIAL; 2: SEARCH; 3: INSERT; 4: EDIT; 5: CLEAR;
    try
...
procedure TTransportadoresDao.ProviderFlags(DataSet: TDataSet);
begin
    try
      {with (DataSet) do
      begin
...
procedure TTransportadoresDao.Save(Model: TTransportadoresModel);
begin
    try
