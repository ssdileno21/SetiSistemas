      ...
      begin
        ...
        begin
          ...
          if (GetFieldInfo('FIELDTYPE_',pSelField) = SConfigFireSMALLINT) or
            ...
            pType := 'NUMBER';
          //SETANDO INFORMAÇÕES DO TIPO DO FIELD SELECINADO
          SetSelectFieldType(GetFieldInfo('FIELDTYPE_',pSelField));
          //SETANDO INFORMAÇÕES DO TAMANHO DO FILTRO SELECIONADO
          SetSelectedFieldSize(GetFieldInfo('FIELDLENGTH_',pSelField));
          //SETANDO INFORMAÇÕES OO NOME DO FIELD SELECIONADO
          ...
          //SE O CAMPO SELECIONADO NÃO FOR VALOR FIXO
          begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (qSelectedFieldFixed = False) then
            begin
              //----- CAMPOS DE DATA EXTRA: TEM PERÍODO SELECIONADO --------//
              if (qPeriod) and (SettingTheFieldType(2) = False) then
              begin
                if (qNullDates) then
                  SetiSqlHelp.SQL.Add(' AND ' + qFieldPeriod+ ' IS NULL') else
              ...
              //- OS CAMPOS DE TEXTO OU NÚMERICOS SERÃO NULO OU NOT NULO ? -//
              ...
              //---------- CAMPOS DE TEXTO = OU <> NÃO INFORMADO ? ---------//
              if (SettingTheFieldType(0)) then
              begin
                if (qNotInformedValues) then
                  SetiSqlHelp.SQL.Add(' AND ' +
                    qSelectedFieldName+' = ''NÃO INFORMADO''');
    ...
    try
      ...
      begin
        ...
        if (qPeriod) and (qSelectFieldType <> SConfigFireTIMESTAMP) and
          (qFieldPeriod = '') then
        begin
          SetiMens(2,'Favor selecionar um campo referente ao período.');
