...
uses
  ...
  Data.FMTBcd, Data.DB, Data.SqlExpr, grimgctrl

  ,uSetiConnects, qrFramelines, qrpctrls, Vcl.Imaging.pngimage,
...
begin
    ...
    if (qPedidoId = -1) then
    begin
      SetiMens(2,'Venda não selecionada.');
      Exit;
    end;
    if (qClienteId = -1) then
    begin
      SetiMens(2,'Cliente não selecionado.');
      Exit;
    end;
