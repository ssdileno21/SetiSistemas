...
var
  sql : TSQLQuery;
begin
  if SetiMensQuestions(0,'Confirma exclusão do serviço?') then
  begin
    try
      qConnects := TSetiConnects.Create;
      sql := TSQLQuery.Create(Nil);
...
begin
    try
      ...
      vSql.ExecSQL;
      SetiMens(1,'Serviço inserido com sucesso.');
    finally
      FreeAndNil(vSql);
      qConnects.Free;
      SetModo('SEARCH');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SetiMens(1,'Informe pelo menos uma observação.');
          Exit;
        end;
      end;
      if edtVlrServico.Text = '' then
      begin
        SetiMens(1,'Valor do serviço não informado.');
      ...
      begin
        ...
        begin
          SetiMens(1,'Valor do serviço não permitido.');
          edtVlrServico.SetFocus;
          Exit;
        end;
      end;
      ...
      begin
        ...
        begin
          ...
          FieldByName('ID_CADPRODUTODEFEITO').DisplayLabel
            := 'Cod.Defeito Prod.';
          FieldByName('ID_FUNCIONARIO').DisplayLabel := 'Cód.Técnico';
          FieldByName('OBSERVACAO').DisplayLabel := 'OBS.';
          FieldByName('VLRSERVICOS').DisplayLabel := 'Vlr.Serviços';
      ...
      }
{
  ...
  não é possivel usar este botao, pois a classe de pesquisa utiliza o mesmo
  dataset da tela principal, sendo assim, com a tela esta em modo de alteração
  ou insercao, e a tela de pesquisa é aberta, quando for gravar a tela principal
  é estorado um erro, dizendo que o dataset não esta criado, uma vez que o mesmo
  é destuido ao fechar a tela de pesquisa...
