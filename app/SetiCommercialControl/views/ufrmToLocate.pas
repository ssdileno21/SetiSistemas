...
uses
  ...
  Vcl.Grids, Vcl.DBGrids, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.Buttons, uFSUF,
  Data.DB, Datasnap.DBClient, uFSDialogs, MIDASLIB, uSetiConfig, Vcl.ImgList,
  VCLTee.TeCanvas, VCLTee.TeePenDlg, Vcl.DBCtrls, uLocateDAO, uSetiUseful;
...
uses udmMasterDataModule, udmLocate;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (SetiCds.Fields[i].DataType = ftSingle) then
        TFloatField(SetiCds.Fields[i]).DisplayFormat := '#,##0.00';
      if (SetiCds.Fields[i].DataType = ftFMTBcd) then
...
begin
    ...
    begin
      if NOT SetiMensQuestions('Alta quantidade de registros,   '+
                             'este procedimento pode demorar, '+
                             'deseja realmente gerar a planilha?',0) then
        v := False;
    end;
    if v then
...
begin
    try
      ...
      if (edtText.Text = '') then
      begin
        if SetiMensQuestions(
          'Não foi informado nenhum valor para a pesquisa, '+
          'a consulta pode demorar, deseja continuar? ',0
          ) then
        begin
          if (qSearchType = 0) then
          begin
